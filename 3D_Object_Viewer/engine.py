...
class QGLControllerWidget(QtOpenGL.QGLWidget):
    ...
    def set_mesh(self, new_mesh):
        ...
        # Use trimesh to extract face and vertex data
        vertices = numpy.array(self.mesh.vertices, dtype='f4')
        faces = numpy.array(self.mesh.faces, dtype='u4')

        # Creates an index buffer for face data
        index_buffer = self.ctx.buffer(faces.tobytes())

        # Creates a list of vertex buffer objects (VBOs)
        vao_content = [(self.ctx.buffer(vertices.tobytes()), '3f', 'in_position')]
    ...
    def init_arcball(self):
        # Create ArcBall
        self.arc_ball = ArcBallUtil(self.width(), self.height())
        mesh_points = self.mesh.vertices
